...
while True:
    ...
    if recievedMessage != "Request to close Connection":
        ...
        if last_recieved_packet + 1 != sequence_number:
            # duplicate detected
            print("Discarding packet {} because Last recieved is {}".format(sequence_number , last_recieved_packet))
            duplicates += 1
            message = str.encode("Recieved packet number " + str(sequence_number))
            socket_udp.sendto(message, senderAddress)


        else:
            print("Recieved packet number: " + str(sequence_number))
            last_recieved_packet = sequence_number
            message = str.encode("Recieved packet number: " + str(sequence_number))
            socket_udp.sendto(message, senderAddress)
