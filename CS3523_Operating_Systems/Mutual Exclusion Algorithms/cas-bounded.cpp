...
// defining an atomic bool array telling us if the thread has been waiting
atomic<bool>* waiting;
...
class params
{
    ...
    params(int N , int K , int thread_number , double lambda1 , double lambda2 , FILE* (f)):
        N(N) , K(K) , thread_number(thread_number), lambda1(lambda1) , lambda2(lambda2) , f(f)  {}
};
...
int main()
{
    ...
    FILE* f = fopen("output_cas-bounded.txt" , "w");

    // printing the heading
    fprintf(f , "Summary for Compare and Swap- Bounded for %d threads and %d Crticial Section Requests\n" , num_threads , num_cs);
    fprintf(f , "\n");
    ...
    // cout << "   Compare and Swap- Bounded" << endl;
...
void* testCS(void* parameters)
{
    ...
    {    
        ...
        while (waiting[id] && !key)
        {
            ...
            If key were true, and flag == expected, then flag becomes true, and key remains true. However, 
            in this scenario, we wish for it to break out of the loop, so we set key to false.
            */
            bool expected = false;
            bool new_val = true;
            key = flag.compare_exchange_strong(expected , new_val);
        }
        waiting[id] = false;
